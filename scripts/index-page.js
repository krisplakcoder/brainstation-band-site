let commentArray = [
    {name:"Connor Walton", date:"02/17/2021", comment:"This is art. This is inexplicable magic expressed in the purest way, everything that makes up this majestic work deserves reverence. Let us appreciate this for what it is and what it contains."},
    {name:"Emilie Beach", date:"01/09/2021", comment:"I feel blessed to have seen them in person. What a show! They were just perfection. If there was one day of my life I could relive, this would be it. What an incredible day."},
    {name:"Miles Acosta", date:"12/20/2020", comment:"I can't stop listening. Every time I hear one of their songs - the vocals - it gives me goosebumps. Shivers straight down my spine. What a beautiful expression of creativity. Can't get enough."}
]

let commSect = document.getElementById("comment-area");
let listSect = document.createElement('ul');
let nameSect = document.createElement('li');

commSect.appendChild(listSect);

function commentSection() {

    for (i= 0; i < commentArray.length; i++) {
        let nameSect = document.createElement('li');
        listSect.appendChild(nameSect);

        let namePara = document.createElement('p');
        nameSect.appendChild(namePara);
        namePara.innerText = commentArray[i].name;

        let datePara = document.createElement('p');
        nameSect.appendChild(datePara);
        datePara.innerText = commentArray[i].date;

        let commentPara = document.createElement('p');
        nameSect.appendChild(commentPara);
        commentPara.innerText = commentArray[i].comment;
        } 
    }



commentSection();

let commentFormEl = document.querySelector(".comment__form");

commentFormEl.addEventListener('submit', (event) => {

    event.preventDefault();

    console.log(event.target.comment.value);
    console.log(event.target.commentSection.value);
    
   
    let newTime = new Date();

    const newComment = {
        name: event.target.comment.value,
        comment: event.target.commentSection.value,
        date: newTime.toString(),
      };

    

    commentArray.unshift(newComment);

    listSect.innerHTML = " ";

    commentSection();

    commentFormEl.reset();
})